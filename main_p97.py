...
init.set_forcefield()
# init.set_forcefield(psf_file="data/P97/5ftm.psf")
...
threshold=4
gaussian_sigma=2
init_density = Volume.from_coords(init.coords, size=size, voxel_size=sampling_rate, sigma=gaussian_sigma, threshold=threshold)
# target_density.show()
# src.viewers.chimera_fit_viewer(init, target_density, genfile="data/P97/5ftm.pdb", ca=True)

target_density = Volume.from_file('data/P97/emd_3299_128_filtered.mrc', voxel_size=sampling_rate, sigma=gaussian_sigma, threshold=threshold)
target_density.rescale(init_density)
# #
# h1  = np.histogram(init_density.data.flatten(),bins=100)
# h2  = np.histogram(target_density.data.flatten(),bins=100)
#
# plt.figure()
...
params ={
    ...
    "x_mass" : 1,#np.sqrt(K_BOLTZMANN*T /CARBON_MASS),
    "q_mass" : 10,
}
n_iter=10
n_warmup = n_iter // 2
...
fit1  =FlexibleFitting(init=init, target=target_density, mode="HMCNMA", params=params, n_iter=n_iter, n_warmup=n_warmup)
fit1.HMC(n_chain=n_chain)

fit2  =FlexibleFitting(init=init, target=target_density, mode="HMC", params=params, n_iter=n_iter, n_warmup=n_warmup)
fit2.HMC(n_chain=n_chain)

fit3  =FlexibleFitting(init=init, target=target_density, mode="NMA", params=params, n_iter=n_iter, n_warmup=n_warmup)
fit3.HMC(n_chain=n_chain)


cc_init= cross_correlation(init_density.data, target_density.data)
L1 = np.cumsum(([1] + fit1.fit[0]["L"])).astype(int) - 1
L2 = np.cumsum(([1] + fit2.fit[0]["L"])).astype(int) - 1
L3 = np.cumsum(([1] + fit3.fit[0]["L"])).astype(int) - 1
fig, ax = plt.subplots(1,1, figsize=(5,2))
ax.plot(np.array([cc_init]+fit1.fit[0]["CC"]), '-', color="tab:red", label=r"$\Delta \mathbf{r}_{local}$ " +"\n"+r"+ $\Delta \mathbf{r}_{global}$")
ax.plot(np.array([cc_init]+fit2.fit[0]["CC"]), '-', color="tab:green", label=r"$\Delta \mathbf{r}_{local}$")
ax.plot(np.array([cc_init]+fit3.fit[0]["CC"]), '-', color="tab:blue", label=r"$\Delta \mathbf{r}_{global}$")
...
# fig.savefig('results/EUSIPCO/HMCNMA_emd_allatoms6'+str(number)+'.png', format='png', dpi=1000)
# fit1.save(  'results/EUSIPCO/HMCNMA_emd_allatoms6'+str(number)+'_fit1.pkl')
# fit2.save(  'results/EUSIPCO/HMCNMA_emd_allatoms6'+str(number)+'_fit2.pkl')
# fit3.save(  'results/EUSIPCO/HMCNMA_emd_allatoms6'+str(number)+'_fit3.pkl')
