...
# def get_F_bonds(coord, bonds, prm):
#     n_atoms = coord.shape[0]
#     f = np.zeros((n_atoms,3))
#     dx = coord[bonds[:, 0]] - coord[bonds[:, 1]]
#     r = np.linalg.norm(dx, axis=1)
#
#     tmp = prm.Kb* (1 - prm.b0 / r)
#     fa = 2 * dx * np.array([tmp,tmp, tmp]).T
#     f [bonds[:, 0]] += fa
#     f [bonds[:, 1]] += -fa
#
#     return f
...
def get_energy_dihedrals(coord, dihedrals, prm):
    ...
    torsions = npg.arctan2(npg.linalg.norm(u2, axis=1) * npg.sum(u1 * npg.cross(u2, u3), axis=1),
                           npg.sum(npg.cross(u1, u2) * npg.cross(u2, u3), axis=1))
    return npg.sum(prm.Kchi * (1 + npg.cos(prm.n * (torsions) - (prm.delta*np.pi/180))))

def get_energy(mol, coord,  verbose=True):
